...
Both new DHW entities, the existing `water_heater.hot_water_tank` entity, and the `switch.hot_water_tank_thermal_disinfect` switch SHALL all be attached to a dedicated "Hot Water Tank" device with identifier `(DOMAIN, f"{uuid}_dhw")` and `via_device=(DOMAIN, uuid)` so HA renders them as a child of the EasyControl Gateway.

#### Scenario: Device association

- **WHEN** HA's device registry is inspected for the POINTTAPI config entry
- **THEN** `sensor.hot_water_tank_actual_temperature`, `binary_sensor.hot_water_tank_heating`, `water_heater.hot_water_tank`, and `switch.hot_water_tank_thermal_disinfect` SHALL all list `identifiers={(DOMAIN, f"{uuid}_dhw")}` matching the Hot Water Tank device, and that device SHALL have `via_device=(DOMAIN, uuid)`
