...
The integration SHALL group POINTTAPI entities into devices that map to physical things the homeowner can identify: **EasyControl Gateway** (`(DOMAIN, uuid)`), **Boiler** (`(DOMAIN, f"{uuid}_boiler")`), **Hot Water Tank** (`(DOMAIN, f"{uuid}_dhw")`), **Heating Zone {zone_id}** (`(DOMAIN, f"{uuid}_zone_{zone_id}")` per zone), and optionally **Solar** (`(DOMAIN, f"{uuid}_solar")`). Every non-gateway device SHALL set `via_device=(DOMAIN, uuid)` so HA renders the gateway as the parent.
...
- **THEN** its `device_info.identifiers` SHALL be `(DOMAIN, f"{uuid}_zone_zn1")` (or the appropriate zone id), NOT `(DOMAIN, uuid)`
...
- **THEN** its `device_info.identifiers` SHALL be `(DOMAIN, f"{uuid}_zone_zn1")`
...
- **THEN** the integration SHALL create a "Heating Zone zn2" device with identifier `(DOMAIN, f"{uuid}_zone_zn2")` and attach the corresponding climate entity to it (not to `(DOMAIN, uuid)`)
