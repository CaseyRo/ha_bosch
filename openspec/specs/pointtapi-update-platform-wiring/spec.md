...
Establish the POINTTAPI branch of the Home Assistant `update` platform — parallel to the `binary_sensor` wiring from v0.31.0. Defines the table-driven `BoschPoinTTAPIUpdateEntityDescription` pattern with `installed_version_fn` / `latest_version_fn` callables so future Update entities can be added with one description entry.
