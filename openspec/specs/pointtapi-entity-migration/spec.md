...
One-shot `async_migrate_entry` that renames legacy entity_ids from v0.30.x into the v0.31.0 device-partition scheme so existing user automations, scripts, and scenes referencing the registry continue to work after upgrade. Idempotent, fault-tolerant, and one-way (no rollback path).
