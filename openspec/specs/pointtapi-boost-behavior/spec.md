...
Codifies the POINTTAPI boost switch behavior. Turn-on first probes for native Bosch boost activation (`boostShortcut`, then `boostZones` + `boostMode`); when a route works the device owns duration and countdown and Home Assistant schedules nothing locally. When no native route works, the integration falls back to a local-state workaround: the zone goes into `manual` mode at `boostTemperature`, an in-memory `BoostSession` is recorded on the coordinator, and an `async_call_later` auto-off fires after `boostDuration` hours; turn-off cancels the timer and restores the zone's prior `userMode`. The `boost_remaining_time` sensor prefers the session's synthetic countdown and otherwise reports Bosch's own `boostRemainingTime`. The probe verdict is cached for the lifetime of the config entry and surfaced in diagnostics. Restart drops the fallback timer (documented limitation).
