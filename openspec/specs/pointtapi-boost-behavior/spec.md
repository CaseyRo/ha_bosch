...
Codifies the POINTTAPI boost switch behavior: a local-state workaround for Bosch's 403-blocked `/heatingCircuits/hc1/boostMode` endpoint. Turn-on puts the zone into `manual` mode at `boostTemperature`, records an in-memory `BoostSession` on the coordinator, and schedules an `async_call_later` auto-off after `boostDuration` hours. Turn-off cancels the timer and restores the zone's prior `userMode`. The `boost_remaining_time` sensor derives a synthetic countdown from the session, falling back to Bosch's reported value when no local session is active. Restart drops the timer (documented limitation).
