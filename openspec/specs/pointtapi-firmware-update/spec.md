...
Surface the EasyControl gateway firmware in Home Assistant's native Updates panel via the `update` platform, plus two diagnostic timestamp sensors for last-check and last-applied times. Read-only — Bosch doesn't expose a programmatic install path; users still trigger updates from the EasyControl app.
